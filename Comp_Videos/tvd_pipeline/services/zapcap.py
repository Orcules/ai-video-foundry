"""ZapCap service for adding subtitles to videos."""
...
class ZapCapService:
    ...
    def _get_random_template_id(self) -> str:
        """Get a random template ID from the config list.

        Returns:
            A random template ID string.
        """
        zc_config = get_zapcap_config()
        return random.choice(zc_config["template_ids"])
    ...
    def _apply_enrichments(
        ...
        enrichments: Optional[Dict[int, Dict]],
    ) -> List[Dict]:
        """Merge emoji/important annotations into normalized BYOT entries.

        Parameters
        ----------
        transcript : list[dict]
            Normalized BYOT entries (text, type, start_time, end_time).
        enrichments : dict[int, dict] | None
            Mapping of word index -> {"emoji": "...", "important": True}.
            If None, returns transcript unchanged.

        Returns
        -------
        list[dict]
            The same entries, with optional ``emoji`` and ``important`` fields added.
        """
        if not enrichments:
            return transcript
        for idx, annotation in enrichments.items():
            if 0 <= idx < len(transcript):
                if "emoji" in annotation:
                    transcript[idx]["emoji"] = annotation["emoji"]
                if annotation.get("important"):
                    transcript[idx]["important"] = True
        annotated_count = sum(
            1 for e in transcript if e.get("emoji") or e.get("important")
        )
        logger.info("   ZapCap BYOT: %d/%d words enriched with emoji/important", annotated_count, len(transcript))
        return transcript
    ...
    def add_subtitles(
        ...
        Args:
            ...
            enrichments: Optional dict mapping word index -> {"emoji": "...", "important": True}.
                        Merged into the normalized transcript before sending to ZapCap.
        ...
        try:
            ...
            # Apply emoji/important enrichments from LLM annotation
            if zapcap_transcript and enrichments:
                zapcap_transcript = self._apply_enrichments(zapcap_transcript, enrichments)
            ...
            try:
                ...
                for upload_attempt in range(2):  # Retry once on timeout
                    try:
                        with open(temp_path, "rb") as f:
                            ...
                            log_external_api_call("zapcap", "upload_video", method="POST", url_hint="/videos")
                            ...
                            _up_ms = int((time.perf_counter() - _up_t0) * 1000)
                            log_external_api_result(
                                ...
                                error=(response.text or "")[:120] if response.status_code not in (200, 201) else "",
                            )
                        upload_ok = True
                        break
                ...
                # Build renderOptions from config
                zc_config = get_zapcap_config()
                ...
                # Only include styleOptions when style_override is enabled
                if zc_config.get("style_override", False):
                    position_map = zc_config.get("subtitle_position_map", {"top": 15, "middle": 50, "bottom": 80})
                    style_opts = dict(render_cfg.get("style_options", {}))
                    style_opts["top"] = position_map.get(subtitle_position, 50)
                    render_options["styleOptions"] = style_opts
                elif zc_config.get("position_only_override", False) and subtitle_position:
                    # Partial override: send only position (top %), preserving template font/color variety
                    position_map = zc_config.get("subtitle_position_map", {"top": 15, "middle": 50, "bottom": 80})
                    render_options["styleOptions"] = {"top": position_map.get(subtitle_position, 50)}
                ...
                log_external_api_call("zapcap", "create_caption_task", method="POST", url_hint=f"/videos/{video_id}/task")
                ...
                _tk_ms = int((time.perf_counter() - _tk_t0) * 1000)
                log_external_api_result(
                    "zapcap",
                    "create_caption_task",
                    duration_ms=_tk_ms,
                    ...
                    ok=task_response.status_code in (200, 201),
                    error=(task_response.text or "")[:120] if task_response.status_code not in (200, 201) else "",
                )
    ...
    def _wait_for_completion(self, video_id: str, task_id: str, timeout: int = 600) -> Optional[str]:
        ...
        log_external_api_call(
            ...
            "task_status_poll",
            method="GET",
            url_hint=f"/videos/{video_id}/task/…",
            detail=f"task_id={str(task_id)[:16]}",
        )
