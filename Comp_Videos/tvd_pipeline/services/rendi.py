"""Rendi.dev service for FFmpeg operations (trim, concat, add VO/music)."""
...
class RendiService:
    ...
    def _x264_video_encode_flags(self) -> str:
        """libx264 arguments for broad phone compatibility (H.264 + 4:2:0)."""
        preset = getattr(config, "RENDI_X264_PRESET", "fast") or "fast"
        prof = getattr(config, "RENDI_X264_PROFILE", "main") or "main"
        crf = int(config.VIDEO_CRF)
        lvl = (getattr(config, "RENDI_X264_LEVEL", "") or "").strip()
        level_arg = f" -level {lvl}" if lvl else ""
        return (
            f"-c:v libx264 -preset {preset} -profile:v {prof}{level_arg} "
            f"-crf {crf} -pix_fmt yuv420p"
        )

    def transcode_social_sharing_mp4(self, video_url: str) -> Optional[str]:
        """Re-encode to H.264 + AAC MP4 for WhatsApp-style players; no-op if disabled or on failure."""
        if not video_url:
            return None
        if not bool(getattr(config, "RENDI_TRANSCODE_AFTER_ZAPCAP", True)):
            return video_url
        try:
            logger.info("📱 Rendi: transcoding to mobile-friendly MP4 (H.264 Main/Baseline + yuv420p + AAC)...")
            url_api = f"{self.base_url}/v1/run-ffmpeg-command"
            vflags = self._x264_video_encode_flags()
            has_audio = self.validate_video_has_audio(video_url)
            if has_audio:
                ffmpeg_command = (
                    f"-i {{{{in_1}}}} -map 0:v:0 -map 0:a:0 {vflags} "
                    f"-c:a aac -b:a 128k -movflags +faststart {{{{out_1}}}}"
                )
            else:
                ffmpeg_command = (
                    f"-i {{{{in_1}}}} -map 0:v:0 {vflags} -an -movflags +faststart {{{{out_1}}}}"
                )
            payload = {
                "input_files": {"in_1": video_url},
                "output_files": {"out_1": "mobile_friendly.mp4"},
                "ffmpeg_command": ffmpeg_command,
                "max_command_run_seconds": 900,
                "vcpu_count": 8,
            }
            response = self._post_json(url_api, payload, 60)
            response.raise_for_status()
            result = response.json()
            if "command_id" not in result:
                logger.warning(f"⚠️ Mobile transcode: Rendi error {result}, using original URL")
                return video_url
            out = self._wait_for_command(result["command_id"])
            if out:
                logger.info("✅ Mobile-friendly transcode complete")
                return out
            logger.warning("⚠️ Mobile transcode returned no URL, using original")
            return video_url
        except Exception as e:
            logger.warning(f"⚠️ Mobile transcode failed (using original): {e}")
            return video_url

    def _post_json(self, url: str, payload: dict, timeout: int = 60):
        """POST JSON to Rendi and log [ExternalAPI] for server-side tracing."""
        from tvd_pipeline.external_api_log import log_external_api_call, log_external_api_result

        try:
            path = url.replace(self.base_url, "").split("?")[0] or url[-100:]
        except Exception:
            path = url[-100:]
        seg = (path.strip("/").split("/")[-1] or "post")[:48]
        log_external_api_call("rendi", seg, method="POST", url_hint=path[:120])
        transient = (
        ...
        for attempt in range(3):
            try:
                ...
                if attempt < 2:
                    wait_s = 1.0 + float(attempt)
                    logger.warning(
                        "Rendi POST transient error (%s), retry %s/3 in %.1fs: %s",
                        type(e).__name__,
                        attempt + 2,
                        wait_s,
                        e,
    ...
    def detect_scenes_cloud(self, video_url: str, threshold: float = 0.1) -> List[float]:
        ...
        try:
            ...
            response = self._post_json(url, payload, 60)
            response.raise_for_status()
    ...
    def apply_ffmpeg_filter(self, video_url: str, filter_string: str) -> Optional[str]:
        """Apply an arbitrary FFmpeg video filter to a video using Rendi.dev.

        Useful for post-processing effects like film grain, color correction, etc.

        Args:
            video_url: URL of the video to process.
            filter_string: FFmpeg -vf filter string (e.g. ``"noise=c0s=3:c0f=t"``).

        Returns:
            URL of the processed video, or None if failed.
        """
        try:
            logger.info(f"🎬 Applying FFmpeg filter via Rendi.dev: {filter_string}")
            ...
            payload = {
                "input_files": {"in_1": video_url},
                "output_files": {"out_1": "filtered_video.mp4"},
                "ffmpeg_command": ffmpeg_command,
                "max_command_run_seconds": 300,
                "vcpu_count": 4,
            }
            ...
            if "command_id" in result:
                command_id = result["command_id"]
                logger.info(f"✅ Filter task created: {command_id}")
                return self._wait_for_command(command_id)
            else:
                logger.error(f"❌ Rendi filter error: {result}")
                return None

        except Exception as e:
            logger.error(f"❌ Error applying FFmpeg filter: {e}")
            return None

    def trim_video(self, video_url: str, duration: float, has_audio: Optional[bool] = None,
                   start_time: float = 0.0) -> Optional[str]:
        ...
        Args:
            ...
            start_time: Start time in seconds to seek to before trimming (default 0.0).

        Returns:
            URL of the trimmed video, or None if failed.
        """
        try:
            logger.info(f"✂️ Trimming video to {duration:.2f}s from {start_time:.2f}s (with re-encode for clean cuts)...")

            url = f"{self.base_url}/v1/run-ffmpeg-command"

            if has_audio is None:
                has_audio = self.validate_video_has_audio(video_url)
            ss_flag = f"-ss {start_time:.3f} " if start_time > 0 else ""
            vflags = self._x264_video_encode_flags()
            if has_audio:
                ffmpeg_command = (
                    f"-i {{{{in_1}}}} {ss_flag}-t {duration:.3f} "
                    f"-map 0:v -map 0:a {vflags} -c:a aac -b:a 128k "
    ...
    # Last scene: centered frame, very subtle zoom only (so viewer sees the whole scene)
    _KB_LAST_SCENE_EFFECT = {
        "name": "last_scene_subtle_zoom",
        "z": "1.0+0.03*on/({frames})",  # Very subtle zoom 1.0 -> 1.03, centered
        "x": "iw/2-(iw/(1.0+0.03*on/({frames}))/2)",
        "y": "ih/2-(ih/(1.0+0.03*on/({frames}))/2)",
    }
    ...
    def create_video_from_image(
        ...
        Args:
            ...
            subtle_for_last_scene: If True, use a very subtle centered zoom (1.0->1.03) so the
        ...
        try:
            ...
            ffmpeg_command = (
                ...
                f"-t {duration:.3f} {self._x264_video_encode_flags()} "
            ...
            if keep_audio:
                ...
                if target_duration:
                    ffmpeg_command = (
                        ...
                        f"{vflags} -c:a aac -b:a 128k -movflags +faststart {{{{out_1}}}}"
                    )
                else:
                    ffmpeg_command = (
                        f"-i {{{{in_1}}}} -filter_complex \"[0:v]setpts={pts_factor:.4f}*PTS[v];[0:a]atempo={atempo_val:.4f}[a]\" "
                        f"-map \"[v]\" -map \"[a]\" {vflags} -c:a aac -b:a 128k -movflags +faststart {{{{out_1}}}}"
                    )
            else:
                ...
                if target_duration:
                    ffmpeg_command = (
                        ...
                        f"-an {vflags} -movflags +faststart {{{{out_1}}}}"
                    )
                else:
                    ffmpeg_command = (
        ...
        def trim_single(item: Dict[str, Any]) -> Dict[str, Any]:
            ...
            try:
                ...
                if target_duration < actual_duration:
                    ...
                    max_slowdown = 1.4  # Max 40% slower — beyond this looks unnatural
    ...
    def concatenate_videos(
        ...
        try:
            ...
            durations = [item.get("duration") if isinstance(item, dict) else 5.0 for item in filtered_video_data]
            # offset_duration: actual clip length for dissolve timing (used when duration is None for pre-trimmed clips)
            offset_durations = [
                (item.get("offset_duration") or item.get("duration") or 4.0) if isinstance(item, dict) else 5.0
                for item in filtered_video_data
            ]

            num_videos = len(video_urls)
            logger.info(f"🎬 Concatenating {num_videos} videos with Rendi (transitions={use_transitions})...")
            for i, dur in enumerate(durations):
                logger.info(f"   Video {i+1}: {dur:.2f}s" if dur is not None else f"   Video {i+1}: pre-trimmed (no re-trim)")
            ...
            if video_only:
                ...
                if dissolve_seconds > 0 and num_videos > 1:
                    return self._concatenate_video_only_with_dissolve(video_urls, durations, dissolve_seconds, offset_durations)
    ...
    def _concatenate_video_only_with_dissolve(
        ...
        offset_durations: List[float] = None,
    ) -> Optional[str]:
        ...
        offset_durations: actual clip lengths for dissolve offset calculation (used when duration is None).
        """
        try:
            unique_urls = []
            unique_durations = []
            unique_offset_durations = []
            seen_urls = set()
            for i, url_val in enumerate(video_urls):
                if url_val and url_val not in seen_urls:
                    unique_urls.append(url_val)
                    unique_durations.append(durations[i] if durations and i < len(durations) else None)
                    unique_offset_durations.append(
                        (offset_durations[i] if offset_durations and i < len(offset_durations) else None)
                        or unique_durations[-1] or 4.0
                    )
                    seen_urls.add(url_val)
            ...
            min_dur = min(unique_offset_durations) or 4.0
            d = min(float(dissolve_seconds if dissolve_seconds is not None else getattr(config, "CONCAT_DISSOLVE_SECONDS", 0.075)), min_dur * 0.5, 0.6)
            ...
            cumulative_time = unique_offset_durations[0] - d
            ...
            for i in range(2, num_videos):
                cumulative_time += unique_offset_durations[i - 1] - d
            ...
            # Safety trim: xfade can produce a trailing black frame when offsets don't align
            # exactly with frame boundaries. Trim 2 frames to account for ZapCap re-encode
            # or xfade rounding reintroducing a frame after the initial trim.
            expected_dur = sum(unique_offset_durations) - (num_videos - 1) * d
            safe_frames = max(1, int(expected_dur * 30) - 2)
            safe_dur = safe_frames / 30.0
            filter_parts[-1] = filter_parts[-1].replace("[outv]", "[xfade_out]")
            filter_parts.append(f"[xfade_out]trim=0:{safe_dur:.6f},setpts=PTS-STARTPTS[outv]")
    ...
    def add_audio_to_video(self, video_url: str, audio_url: str, video_duration: float = None) -> Optional[str]:
        """Add audio track to a video.

        Args:
            video_url: URL of the video.
            audio_url: URL of the audio file.
            video_duration: Explicit video duration in seconds. When provided, uses -t
                instead of -shortest to ensure video/audio streams have identical duration.
        ...
        try:
            ...
            duration_flag = f"-t {video_duration:.3f}" if video_duration and video_duration > 0 else "-shortest"
            ffmpeg_command = f"-i {{{{in_1}}}} -i {{{{in_2}}}} -map 0:v -map 1:a -c:v copy -c:a aac -b:a 192k {duration_flag} {{{{out_1}}}}"
    ...
    def validate_video_has_audio(self, video_url: str) -> bool:
        ...
        try:
            ...
            payload = {
                ...
                "output_files": {"out_1": "probe_audio.txt"},
                "vcpu_count": 1,
                "max_command_run_seconds": 30
            }
            
            response = self._post_json(url, payload, 30)
    ...
    def validate_video_has_video_stream(self, video_url: str) -> bool:
        """Check if a video file has a video stream using FFprobe via Rendi.

        Args:
            video_url: URL of the video to check.

        Returns:
            True if video has a video stream, False otherwise.
        """
        try:
            logger.info("🔍 Validating video has video stream...")

            url = f"{self.base_url}/v1/run-ffmpeg-command"

            ffprobe_command = (
                "-v quiet -select_streams v:0 -show_entries stream=codec_type "
                "-of default=noprint_wrappers=1:nokey=1 {{in_1}}"
            )
            ...
            if response.status_code == 200:
                ...
                if status == "failed":
                    logger.warning("⚠️ Video has no video stream (probe confirmed failed)")
                    return False
                if status == "completed" and "video" in str(result).lower():
                    logger.info("✅ Video has video stream")
                    return True

            # Rendi probe inconclusive (e.g. API rejected empty output_files, network error, etc.).
            # Treat as valid — only return False on a confirmed Rendi "failed" status above.
            # Returning False here triggers VeoRAIBlockedError and a wasteful Runway fallback.
            logger.warning("⚠️ Rendi probe non-200 (%s) — assuming video stream present", response.status_code)
            return True

        except Exception as e:
            logger.warning(f"⚠️ Could not validate video stream: {e}, assuming video stream present")
            return True

    def ensure_stereo_audio_track(self, video_url: str) -> Optional[str]:
        """Mux a silent stereo AAC track when the file has no audio stream.

        Used for UGC Real (and similar) when concatenating Kling Avatar clips (with speech)
        next to silent I2V clips: every segment must expose [i:a] for concat filters.

        If the input already has an audio track (per ffprobe), returns the same URL unchanged.
        """
        if not video_url or not str(video_url).strip():
            return None
        try:
            if self.validate_video_has_audio(video_url):
                return video_url
        except Exception as e:
            logger.warning("⚠️ ensure_stereo_audio_track: probe failed (%s); attempting silent mux anyway", e)

        try:
            logger.info("🔇 Muxing silent stereo AAC (no audio stream on clip)")
            url = f"{self.base_url}/v1/run-ffmpeg-command"
            ffmpeg_command = (
                f"-i {{{{in_1}}}} -f lavfi -i anullsrc=channel_layout=stereo:sample_rate=44100 "
                f"-map 0:v -map 1:a -c:v copy -c:a aac -b:a 192k -shortest -movflags +faststart {{{{out_1}}}}"
            )
            payload = {
                "ffmpeg_command": ffmpeg_command,
                "input_files": {"in_1": video_url},
                "output_files": {"out_1": "clip_with_silent_audio.mp4"},
                "vcpu_count": 2,
                "max_command_run_seconds": 300,
            }
            response = self._post_json(url, payload, 60)
            ...
            if "command_id" in result:
                ...
                if out:
                    return out
            logger.error("❌ ensure_stereo_audio_track: Rendi returned no command_id / output")
            return None
        except Exception as e:
            logger.error("❌ ensure_stereo_audio_track failed: %s", e)
            return None
    ...
    def add_background_music_to_video(
        ...
        try:
            ...
            vflags = self._x264_video_encode_flags()
            if not has_audio:
                # Re-encode video so HEVC/high444 from upstream never reaches the final file (WhatsApp-safe AVC).
                ffmpeg_command = (
                    f"-i {{{{in_1}}}} -i {{{{in_2}}}} "
                    f"-map 0:v -map 1:a {vflags} -c:a aac -b:a 192k -shortest -movflags +faststart {{{{out_1}}}}"
                )
            else:
                ...
                ffmpeg_command = (
                    ...
                    f"-map 0:v -map \"[mixed]\" {vflags} -c:a aac -b:a 192k -shortest -movflags +faststart {{{{out_1}}}}"
                )
    ...
    def append_end_card(
        ...
        """Append a video-only end card to a video with audio.

        Uses xfade for visual dissolve. When music_url is provided, continues
        the music (with fade-out) during the end card instead of padding with
        silence.

        Args:
            video_url: URL of the main video (with audio).
            end_card_url: URL of the end card video (video-only).
            video_duration: Duration of the main video in seconds.
            end_card_duration: Duration of the end card in seconds.
            dissolve: Crossfade duration in seconds.
            music_url: Optional URL of the original music track to continue during end card.
            music_volume: Volume level for music continuation (default 0.2, matching main mix).

        Returns:
            URL of the combined video, or None if failed.
        """
        try:
            offset = round(video_duration - dissolve, 3)
            total_dur = round(video_duration + end_card_duration - dissolve, 3)

            logger.info(f"🎬 Appending end card: {video_duration:.1f}s + {end_card_duration:.1f}s (dissolve={dissolve}s) → ~{total_dur:.1f}s"
                        f"{' [with music continuation]' if music_url else ''}")

            url = f"{self.base_url}/v1/run-ffmpeg-command"
            vflags = self._x264_video_encode_flags()

            if music_url:
                # 3-input command: continue music during end card with fade-out
                seek_pos = round(max(video_duration - 0.5, 0), 3)
                fade_start = round(max(end_card_duration - 1.5, 0.5), 3)
                fade_dur = round(min(1.5, end_card_duration - 0.3), 3)
                ec_dur = round(end_card_duration, 3)

                ffmpeg_command = (
                    f"-i {{{{in_1}}}} -i {{{{in_2}}}} -i {{{{in_3}}}} -filter_complex "
                    f"\"[1:v][0:v]scale2ref[ec][ref];"
                    f"[ref]fps=30[ref2];[ec]fps=30[ec2];"
                    f"[ref2][ec2]xfade=transition=fade:duration={dissolve}:offset={offset}[v];"
                    f"[2:a]volume={music_volume},"
                    f"atrim=start={seek_pos},asetpts=PTS-STARTPTS,"
                    f"afade=out:st={fade_start}:d={fade_dur},"
                    f"apad=whole_dur={ec_dur},"
                    f"atrim=0:{ec_dur},asetpts=PTS-STARTPTS[ec_music];"
                    f"[0:a][ec_music]concat=n=2:v=0:a=1[a]\" "
                    f"-map \"[v]\" -map \"[a]\" "
                    f"{vflags} "
                    f"-c:a aac -b:a 128k -movflags +faststart "
                    f"-t {total_dur} {{{{out_1}}}}"
                )
                input_files = {"in_1": video_url, "in_2": end_card_url, "in_3": music_url}
            else:
                # 2-input command: pad with silence (original behavior)
                ffmpeg_command = (
                    f"-i {{{{in_1}}}} -i {{{{in_2}}}} -filter_complex "
                    f"\"[1:v][0:v]scale2ref[ec][ref];"
                    f"[ref]fps=30[ref2];[ec]fps=30[ec2];"
                    f"[ref2][ec2]xfade=transition=fade:duration={dissolve}:offset={offset}[v];"
                    f"[0:a]apad=pad_dur={end_card_duration}[a]\" "
                    f"-map \"[v]\" -map \"[a]\" "
                    f"{vflags} "
                    f"-c:a aac -b:a 128k -movflags +faststart "
                    f"-t {total_dur} {{{{out_1}}}}"
                )
                input_files = {"in_1": video_url, "in_2": end_card_url}

            payload = {
                "input_files": input_files,
                "ffmpeg_command": ffmpeg_command,
                "output_files": {"out_1": "end_card_appended.mp4"},
                "vcpu_count": 4,
                "max_command_run_seconds": 180,
            }
            ...
            if "command_id" in result:
                command_id = result["command_id"]
                logger.info(f"✅ Rendi end card append task created: {command_id}")
                output_url = self._wait_for_command(command_id)
                if output_url:
                    logger.info(f"✅ End card appended: {video_duration:.1f}s + {end_card_duration:.1f}s → ~{total_dur:.1f}s")
                    return output_url
                logger.warning(f"❌ End card append: command failed or timed out")
                return None
            else:
                logger.error(f"❌ Rendi API error: {result}")
                return None
            ...
            logger.error(f"❌ End card append error: {e}")
            return None
    ...
    def overlay_logo_on_video(
        ...
        try:
            ...
            if duration_seconds and duration_seconds > 0:
                # Show logo only in last N seconds
                # enable='gte(t,TOTAL_DURATION-N)'
                # We need to get total duration first or use end-relative timing
                # Using negative time reference: logo appears when (total_time - current_time) <= duration_seconds
                # FFmpeg doesn't easily support this, so we use a workaround:
                # Probe video duration first, or use a very large start time that gets clamped
                enable_expr = f":enable='gte(t,main_w-{duration_seconds})'"
                # Actually, we can't know video duration without probing.
                # Better approach: show in last N seconds by checking against a reasonable video length
                # Or just show the whole video if duration_seconds is None
                logger.info(f"   Logo will appear in last {duration_seconds}s of video")
                # For simplicity, let's show logo for the entire video and note that
                # more complex timing requires knowing video duration
            ...
            ffmpeg_command = (
                ...
                f"-map \"[outv]\" -map 0:a? {vflags} -c:a copy -movflags +faststart {{{{out_1}}}}"
            )
    ...
    def add_vo_and_music_to_video(
        ...
        - -t video_duration (or -shortest fallback): output = video length
        ...
        try:
            ...
            # -t video_duration: forces all streams to same length (fixes video/audio mismatch
            #   that breaks downstream xfade in append_end_card)
            # -shortest (fallback): output = min(video, mix) = video length
            duration_flag = f"-t {video_duration:.3f}" if video_duration and video_duration > 0 else "-shortest"
            vflags = self._x264_video_encode_flags()
            ffmpeg_command = (
                ...
                f"-map 0:v -map \"[mixed]\" {vflags} -c:a aac -b:a 192k {duration_flag} -movflags +faststart {{{{out_1}}}}"
            )
    ...
    def overlay_cta_on_video(
        ...
        try:
            ...
            ffmpeg_command = (
                ...
                f"-map \"[out]\" -map 0:a? {vflags} -c:a copy -movflags +faststart {{{{out_1}}}}"
            )
    ...
    def overlay_logo_on_video(
        ...
        """Overlay a logo image on a video.
        
        The logo appears during the last scene or for a specified duration at the end.
        
        Args:
            video_url: URL of the video to overlay on.
            logo_url: URL of the logo image (PNG with transparency recommended).
            position: Position of the logo ('bottom_right', 'bottom_left', 'top_right', 'top_left', 'center').
            logo_scale: Scale of logo relative to video width (0.1 = 10% of video width). Default 0.15.
            start_time: Start time in seconds when logo should appear (None = show for entire video).
            padding_percent: Padding from edges as percentage of video dimensions. Default 0.03 (3%).
        ...
        try:
            ...
            # Calculate padding
            pad_x = f"main_w*{padding_percent}"
            pad_y = f"main_h*{padding_percent}"
            
            # Position calculations based on position parameter
            if position == "bottom_right":
                overlay_x = f"main_w-overlay_w-{pad_x}"
                overlay_y = f"main_h-overlay_h-{pad_y}"
            ...
            overlay_filter = f"overlay=x={overlay_x}:y={overlay_y}"
            
            # Add timing condition if start_time is specified
            if start_time is not None:
                overlay_filter += f":enable='gte(t,{start_time})'"
            
            # FFmpeg command to overlay logo image on video
            # Scale the logo based on logo_scale parameter (relative to video width)
            vflags = self._x264_video_encode_flags()
            ffmpeg_command = (
                f"-i {{{{in_1}}}} -i {{{{in_2}}}} "
                f"-filter_complex \"[1:v]scale=iw*{logo_scale}:-1[scaled];[0:v][scaled]{overlay_filter}[out]\" "
                f"-map \"[out]\" -map 0:a? {vflags} -c:a copy -movflags +faststart {{{{out_1}}}}"
            )
            ...
            logger.error(f"❌ Error overlaying logo on video: {e}")
            return None
    ...
    def _wait_for_command(self, command_id: str, timeout: int = 600) -> Optional[str]:
        ...
        """
        start_time = time.time()
        check_interval = getattr(config, "RENDI_POLL_INTERVAL", 10)
        if check_interval < 2:
            check_interval = 2
    ...
    def detect_motion(self, video_url: str, threshold: float = 0.02) -> bool:
        """Detect whether a video clip has meaningful motion.

        Extracts 3 frames (start, middle, end) via Rendi FFmpeg, computes the
        average absolute pixel difference between consecutive pairs, and
        returns ``True`` if any pair exceeds *threshold*.

        Args:
            video_url: URL of the video clip to check.
            threshold: Normalised pixel-difference threshold (0-1). Default 0.02.

        Returns:
            ``True`` if motion is detected, ``False`` if the clip appears static.
            Returns ``True`` on any error (fail-open: don't discard clips on check failure).
        """
        try:
            url = f"{self.base_url}/v1/run-ffmpeg-command"

            # Extract 3 frames (first, middle, last) and compute pixel diffs.
            # Output a single text file with two diff values (pair 0-1, pair 1-2).
            filter_complex = (
                "[0:v]select='eq(n\\,0)+eq(n\\,floor(t*30/2))+eq(n\\,floor(t*30-1))',"
                "scale=160:90,format=gray,split=3[a][b][c];"
                "[a][b]blend=all_mode=difference[d1];"
                "[b][c]blend=all_mode=difference[d2];"
                "[d1]blackframe=amount=100:threshold=24[bf1];"
                "[d2]blackframe=amount=100:threshold=24[bf2]"
            )

            # Simpler approach: extract 3 PNGs, compare file sizes.
            # Even simpler: use a single ffmpeg pass that outputs per-frame mean brightness
            # of the difference frames.  If both diffs are near-black → static.
            #
            # Most reliable Rendi-compatible approach: output 3 thumbnail PNGs, then
            # compare them externally.  But Rendi only supports single-command FFmpeg.
            #
            # Practical solution: extract first + last frame as thumbnails, compute
            # SSIM or just rely on file-size delta.  Instead, use the lavfi approach:
            # produce a very short video of the *difference* and check if the output
            # is essentially black.

            # Strategy: produce a 1-frame PNG of the pixel-wise max of all frame diffs.
            # If that PNG is near-black, the clip is static.
            ffmpeg_command = (
                "-i {{in_1}} "
                "-vf \"select='eq(n\\,0)+eq(n\\,15)+eq(n\\,29)',scale=160:90,format=gray\" "
                "-frames:v 3 -f image2 -y {{out_1}}"
            )

            # Actually, the simplest reliable approach with Rendi:
            # Extract frame 0 and frame at end, output two PNGs, compare sizes.
            # Rendi supports multiple outputs.
            ffmpeg_command = (
                "-i {{in_1}} "
                "-vf \"select='eq(n\\,0)',scale=160:90\" -frames:v 1 -update 1 {{out_1}} "
                "-vf \"select='eq(n\\,44)',scale=160:90\" -frames:v 1 -update 1 {{out_2}} "
                "-vf \"select='eq(n\\,89)',scale=160:90\" -frames:v 1 -update 1 {{out_3}}"
            )

            # Rendi-compatible: use -lavfi to compute the mean pixel diff across the whole clip.
            # If std-dev of luma across frames is very low, the clip is static.
            # signalstats outputs per-frame stats; we just need to check variance.
            #
            # Simplest single-command approach: compute the overall PSNR between first and last
            # frame.  Very high PSNR (>50dB) = effectively identical = no motion.
            #
            # Let's use a clean, reliable approach: extract 2 frames, compute blend=difference,
            # output a single-frame JPEG, measure its average brightness via output file size.

            ffmpeg_command = (
                "-i {{in_1}} "
                "-filter_complex "
                "\"[0:v]trim=start=0:end=0.04,setpts=PTS-STARTPTS[first];"
                "[0:v]reverse,trim=start=0:end=0.04,setpts=PTS-STARTPTS[last];"
                "[first][last]blend=all_mode=difference,format=gray,scale=160:90[diff]\" "
                "-map \"[diff]\" -frames:v 1 {{out_1}}"
            )

            payload = {
                "input_files": {"in_1": video_url},
                "output_files": {"out_1": "diff_frame.png"},
                "ffmpeg_command": ffmpeg_command,
                "max_command_run_seconds": 60,
                "vcpu_count": 2,
            }
            ...
            if "command_id" not in result:
                logger.warning("detect_motion: no command_id, assuming motion present")
                return True

            diff_url = self._wait_for_command(result["command_id"])
            if not diff_url:
                logger.warning("detect_motion: diff frame extraction failed, assuming motion present")
                return True

            # Download the diff frame and compute mean brightness
            try:
                img_resp = requests.get(diff_url, timeout=15)
                img_resp.raise_for_status()
                img_bytes = img_resp.content
                # Quick heuristic: average byte value of the raw PNG data
                # (includes header, but for a 160x90 grayscale image the payload
                #  dominates).  A near-black image compresses to very small size.
                # More reliable: compute actual mean of pixel values.
                #
                # Use file size as proxy:
                # - Static clip diff → near-black PNG → very small (< ~500 bytes for 160x90)
                # - Moving clip diff → bright areas → larger file (> 1KB typically)
                file_size = len(img_bytes)
                # For a 160x90 grayscale PNG, a fully black image is ~200-400 bytes.
                # A diff with motion produces 800+ bytes.
                size_threshold = 600  # bytes
                has_motion = file_size > size_threshold
                logger.info(
                    f"detect_motion: diff frame size={file_size} bytes, "
                    f"threshold={size_threshold} → {'MOTION' if has_motion else 'STATIC'}"
                )
                return has_motion
            except Exception as dl_err:
                logger.warning(f"detect_motion: could not download diff frame ({dl_err}), assuming motion present")
                return True

        except Exception as e:
            logger.warning(f"detect_motion: error ({e}), assuming motion present")
            return True
