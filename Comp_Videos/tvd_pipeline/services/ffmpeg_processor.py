...
class FFmpegProcessor:
    ...
    def download_video(video_url: str, output_path: str) -> bool:
        """Download a video from URL or copy a local file (Windows-friendly).
        
        Args:
            video_url: URL of the video to download, or a local filesystem path.
        ...
        try:
            ...
            src = (video_url or "").strip()
            if src.startswith("file://"):
                src = src[7:] if len(src) > 7 else src
                if os.name == "nt" and src.startswith("/"):
                    src = src.lstrip("/")
            if os.path.isfile(src):
                logger.info(f"📥 Copying local video to temp: {src[:50]}...")
