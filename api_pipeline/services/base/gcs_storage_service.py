...
class GCSStorageService:
        ...
        try:
            ...
            if make_public:
                try:
                    ...
                    logger.info(f"ℹ️ ACL not supported, using direct URL")
    ...
    def upload_audio_from_url(
        ...
        Args:
            source_url: URL of the audio to download.
            key_name: Name for the GCS object (e.g. jobs/abc/vo_audio.mp3).
            make_public: Whether to make the object publicly readable.
        ...
        if not source_url or not source_url.strip().startswith(("http://", "https://")):
            return None
        try:
            r = requests.get(source_url, stream=True, timeout=timeout)
            r.raise_for_status()
            ...
            if "wav" in ct:
                ...
                ext = ".mp3"
            if not key_name.lower().endswith((".mp3", ".wav", ".ogg", ".m4a")):
            ...
            logger.warning(f"Could not upload audio from URL to GCS: {e}")
            return None
    ...
    def upload_image_from_url(
        ...
        """Download image from URL and upload to GCS. Use for reference images that return 403 when fetched by external APIs (e.g. Instagram/Facebook CDN).
        ...
        try:
            ...
            logger.warning(f"⚠️ Could not upload image from URL to GCS: {e}")
            return None
